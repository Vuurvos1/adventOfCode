...
/**
 * @param {{size: number, value: number}[]} pairs
 * @returns
 */
function scoreHand(pairs) {
    // get highset non joker pair
    if (pairs.length > 1 && pairs.find((p) => p.value === 1)) {
        const highestPair = pairs.findIndex((p) => p.value !== 1);
        const jokerPair = pairs.find((p) => p.value === 1);
        pairs[highestPair].size += jokerPair.size;

        pairs = pairs.filter((p) => p.value !== 1);
    }

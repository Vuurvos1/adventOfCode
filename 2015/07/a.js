...
for (const line of input.sort((a, b) => a.length - b.length)) {
    ...
    for (let i = 0; i < args.length; i++) {
        ...
        if (arg === "NOT") {
            let tmp = lib.isValidIntegerString(nextArg)
                ? Number(nextArg)
                : values[nextArg];
        ...
        if (i < 1) {
            v1[0] = lib.isValidIntegerString(arg) ? Number(arg) : values[arg];
            val[0] = v1[0];
        } else {
            v2[0] = lib.isValidIntegerString(arg) ? Number(arg) : values[arg];
        }
    }
